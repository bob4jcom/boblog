...
admin.article = {
    ...
    getAndSet: function () {
        ...
        $.ajax({
            ...
            success: function (result, textStatus) {
                ...
                if (result.article.articleAttachments) {
                	while(true) {
                		var next = $("#articleUpload").next();
                		if (next && next.length) {
                			next.remove();
                		} else {
                			break;
                	...
                	var attachments = result.article.articleAttachments.split("@1_1@");
                	for(var i = 0; i < attachments.length; i++) {
                		var kv = attachments[i].split("#2&2#");
                		$("#articleUpload").after("<div class='uploadContent'><a target='_blank' href='"+kv[0]+"'>"+kv[1]+"</a></div>");
                	}
                }
    ...
    add: function (articleIsPublished, isAuto) {
        if (admin.article.validate()) {
            ...
            var articleAttachments = "";
            var attachments = $("#articleUploadDiv .uploadContent").find("a");
            if (attachments && attachments.length) {
            	if (attachments.length.length > 3) {
            		alert("最多上传3个附件");
            		return;
            	}
            	attachments.each(function(i){
            		if (i >0) {
            			articleAttachments += "@1_1@";
            		}
            		articleAttachments = articleAttachments + $(this).attr("href") + "#2&2#" + $(this).text();
            	});
            }
    ...
    init: function (fun) {
        ...
        $('#articleUpload').fileupload({
            ...
            formData: function (form) {
                ...
                $('#articleUpload').after('<div class="uploadContent"><a target="_blank" href="http://' + qiniu.qiniuDomain + qiniuKey + '">[' + filename + ']</a></div>');
            },
