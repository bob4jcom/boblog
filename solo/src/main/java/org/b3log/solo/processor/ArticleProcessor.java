...
public class ArticleProcessor {
    ...
    public void showArticle(final HTTPRequestContext context, final HttpServletRequest request, final HttpServletResponse response)
        ...
        try {
            ...
            String attachments = article.getString(Article.ARTICLE_ATTACHMENTS);
            if (StringUtils.isNotBlank(attachments)) {
            	StringBuilder sb = new StringBuilder("<div><label>附件：</label>");
            	String[] attachmentArr = attachments.split("@1_1@");
            	for (int i = 0; i < attachmentArr.length; i++) {
            		String[] kv = attachmentArr[i].split("#2&2#");
            		sb.append("<br/> <a  target='_blank' href='").append(kv[0]).append("'>").append(kv[1]).append("</a>");
            	}
            	sb.append("</div>");
            	article.put(Article.ARTICLE_ATTACHMENTS, sb.toString());
            }
